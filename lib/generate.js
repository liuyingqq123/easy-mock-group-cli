...
async function init(apis, cb) {
  ...
  projectIds.forEach((projectId) => {
    var api = apis[projectId]
    logger.log('> swaggerapis %s', api.mocks.length)
    const swaggerapis = api.mocks;
    let nameTemp = "";
    for (let i = 0; i < swaggerapis.length; i++) {
      const apiItem = swaggerapis[i];

      var confProject = _.find(config.projects, ['id', projectId])

      // 提取文件夹
      confProject.name = apiItem.url.split(/\//)[1];
      if (nameTemp == confProject.name) {
        continue;
      }
      if (!confProject) return

      var api2 = _.filter(swaggerapis, function(o) {
        if (o.url.indexOf(`/${confProject.name}/`) == 0) {
          return true;
        } else {
          return false;
        }
      })
      // logger.log('> init api2 %s', api2.length)
      var dest = path.join(config.output, confProject.name)
      var data = {
        data: { project: api.project, mocks: api2 },
        config: config,
        project: confProject,
        _: _,
...
async function build (data, source, dest, cb, ignore) {
  ...
  return await metalsmith.build((error, files) => {
