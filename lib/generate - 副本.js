...
function filterAPI (apis) {
  // logger.log('> filterAPI apis %s', JSON.stringify(apis))
  Object.keys(apis).forEach((projectId) => {
    logger.log('> filterAPI projectId %s', projectId)
    var confProjectList = _.filter(config.projects, ['id', projectId])

    logger.log('> filterAPI confProjectList.length %s', confProjectList.length)
    if (confProjectList.length == 0) return

    for (let i = 0; i < confProjectList.length; i++) {
      let confProject = confProjectList[i];
      logger.log('> filterAPI confProject %s', JSON.stringify(confProject))
...
function init (apis, cb) {
  ...
  projectIds.forEach((projectId) => {
    var api = apis[projectId]
    var confProjectList = _.filter(config.projects, ['id', projectId])
    logger.log('> init confProjectList.length %s', confProjectList.length)
    if (confProjectList.length == 0) return

    for (let i = 0; i < confProjectList.length; i++) {
      let confProject = confProjectList[i];
      logger.log('> init confProject %s', JSON.stringify(confProject))
...
function build (data, source, dest, cb, ignore) {
  logger.log('> build data %s', JSON.stringify(data))
  logger.log('> build source %s', source)
  logger.log('> build dest %s', dest)
  logger.log('> build cb %s', cb)
...
module.exports = function (_projectPath, _templatePath, _config, cb) {
  ...
  var projectIdsClone = _.clone(config.projects)
  var projectIds = projectIdsClone
    .filter(o => _.has(o, 'id') && _.has(o, 'name'))
    .map(project => project.id)


  if (projectIds.length !== projectIdsClone.length) logger.fatal('缺少字段，请正确配置项目列表.')
  ...
  axios
    ...
    .then((res) => {
      ...
      logger.log('> axios data %s', JSON.stringify(res.data.data))
      if (data) {
        filterAPI(data)
